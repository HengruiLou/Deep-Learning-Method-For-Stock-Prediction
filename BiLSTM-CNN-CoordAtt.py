...
import os
import random
from tensorflow.keras.layers import Conv2D, AvgPool2D, Input,Lambda,Concatenate,Reshape,Activation,Multiply,BatchNormalization
...
class DataSolve(object):
    ...
    def normalize(self):
        # z-score归一化
         ...
         print(scaler.scale_)
         m=self.test_y
         print(m*scaler.scale_[1]+scaler.mean_[1])
...
class ConstructModel(object):
    ...
    def model(self):
        ...
        conv2D = Conv2D(64, (3, 3),padding='same',kernel_regularizer=l1_l2(0.001, 0.001), bias_regularizer=l1_l2(0.001, 0.001))(BiLstm_out)
        conv2D=BatchNormalization()(conv2D)
        conv2D=Activation('relu')(conv2D)

        attention=CoordAtt(conv2D)
        attention=BatchNormalization()(attention)
        attention = Dropout(0.2)(attention)

        flatten = Flatten()(attention)

        # dense_1 = Dense(20,kernel_regularizer=l1_l2(0.001, 0.001), bias_regularizer=l1_l2(0.001, 0.001))(
        #     flatten)
        # dense_1 = BatchNormalization()(dense_1)
        # dense_1 = Dropout(0.2)(dense_1)
        #
        # dense_2 = Dense(10, kernel_regularizer=l1_l2(0.001, 0.001), bias_regularizer=l1_l2(0.001, 0.001))(
        #     dense_1)
...
class TrainModel(object):
    ...
    def train(self, name):
        ...
        # keras.utils.plot_model(model, "coordAtt.png", show_shapes=True)
        model.fit(train_x,
                  train_y,
        ...
        # pd.DataFrame(y_pred).to_csv("./three_model/000001.csv",
        #                             index=False, header=False)
        return y_true,y_pred

class Main(object):
    std_file_name = "./logs/" + time.strftime("%Y_%b_%d_%H_%M_%S", time.gmtime()) + "_timestep" + str(
        timestep) + "_BiLSTM_CNN_Cord_Stock.txt"
